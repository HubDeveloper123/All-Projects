...
function gameEngine(){
  musicSound.play();
    ...
    if(snakeArr[0].y === obstacle.y && snakeArr[0].x === obstacle.x){
      gameOverSound.play();
      musicSound.pause();
      inputDir =  {x: 0, y: 0}; 
      alert("Game Over. Press any key to play again!");
    ...
    if(snakeArr[0].y === food.y && snakeArr[0].x ===food.x){
        ...
        let c = 3;
        let d = 16;
        obstacle = {x: Math.round(c + (d-c)* Math.random()), y: Math.round(a + (b-a)* Math.random())}
    }
    ...
    // if obstacle x and y == food x and y
        if(obstacle.y === food.y && obstacle.x === food.x){
          let a = 2;
          let b = 16;
          food = {x: Math.round(a + (b-a)* Math.random()), y: Math.round(a + (b-a)*   Math.random())}
          let c = 3;
